...
public class TelaMonitoramento extends javax.swing.JFrame {
    ...
    void configurarComponentes() {        
        ...
        cpu.mascara = "%d%%";
//        cpu.lbAtual = lbAtualCpu;
//        cpu.pbBarra = pbCpu;
//        cpu.lbMinimo = lbMinimoCpu;
//        cpu.lbMaximo = lbMaximoCpu;
//        cpu.lbMedia = lbMediaCpu;
        
        disco.nome = "Disco";
        disco.maximoPossivel = 250;
        disco.minimo =  disco.maximoPossivel;
        disco.mascara = "%dGB";
//        disco.lbAtual = lbAtualDisco;
//        disco.pbBarra = pbDisco;
//        disco.lbMinimo = lbMinimoDisco;
//        disco.lbMaximo = lbMaximoDisco;
//        disco.lbMedia = lbMediaDisco;        
        
        memoria.nome = "Memória";
        memoria.maximoPossivel = 8 * 1024;
        memoria.minimo =  memoria.maximoPossivel;
        memoria.mascara = "%dMB";
//        memoria.lbAtual = lbAtualMemoria;
//        memoria.pbBarra = pbMemoria;
//        memoria.lbMinimo = lbMinimoMemoria;
//        memoria.lbMaximo = lbMaximoMemoria;
//        memoria.lbMedia = lbMediaMemoria;        
    }
    
    void atribuirLeitura(Componente componente, Integer novoValor) {
        ...
        if(componente==cpu){
            atualizarTela(pbCpu,lbAtualCpu,cpu.atual,lbMinimoCpu,componente.minimo,lbMaximoCpu,componente.maximo,lbMediaCpu,componente);
            
        }
        else if(componente==disco){
            atualizarTela(pbDisco,lbAtualDisco,disco.atual,lbMinimoDisco,componente.minimo,lbMaximoDisco,componente.maximo,lbMediaDisco,componente);
        }
        else{
            atualizarTela(pbMemoria,lbAtualMemoria,memoria.atual,lbMinimoMemoria,componente.minimo,lbMaximoMemoria,componente.maximo,lbMediaMemoria,componente);
        }
        
    }
    
    void atualizarTela(JProgressBar barra,JLabel lbAtual,Integer atual,JLabel mini,Integer minimo,JLabel max,Integer maxi,JLabel lbMed,Componente componente) {
        
        barra.setValue(atual);
        
        lbAtual.setText(
                String.format("%d", atual));
        
        mini.setText(
                String.format("%d",minimo));
        
        max.setText(
                String.format("%d",maxi));
        
        Integer medConta =componente.soma/leituras;
        lbMed.setText(
                String.format("%d",medConta));
    }
    ...
    private void btLeituraActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btLeituraActionPerformed
        ...
        atribuirLeitura(cpu, leituraCpu);
        atribuirLeitura(disco, leituraDisco);
        atribuirLeitura(memoria, leituraMemoria);
